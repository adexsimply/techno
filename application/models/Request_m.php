...
class Request_m extends CI_Model
{
    ...
    {
        $get_lab_request = $this->db->select('r.*,p.patient_title,p.patient_name,p.patient_status,p.patient_gender,p.patient_id_num,s.staff_firstname,s.staff_lastname,c.clinic_name, pr.id as request_id')->from('radiology_request r')->join('patient_details as p', 'p.id=r.patient_id', 'left')->join('staff as s', 's.user_id=r.sender_id', 'left')->join('patient_radiology_tests as pr', 'pr.radiology_test_unique_id=r.radiology_test_unique_id ', 'left')->join('patient_vitals as pv', 'pv.patient_id=r.patient_id', 'left')->join('clinics as c', 'c.id=pv.clinic_id', 'left')->get();
        $lab_request_list = $get_lab_request->result();
        return $lab_request_list;
    }
    public function get_rad_request_list_default()
    {
        $get_lab_request = $this->db->select('pr.*,p.patient_title,p.patient_name,p.patient_status,p.patient_gender,p.patient_id_num,s.staff_firstname,s.staff_lastname,c.clinic_name, pr.id as request_id')->from('patient_radiology_tests pr')->join('patient_details as p', 'p.id=pr.patient_id', 'left')->join('staff as s', 's.user_id=pr.sender_id', 'left')->join('patient_vitals as pv', 'pv.patient_id=pr.patient_id', 'left')->join('clinics as c', 'c.id=pv.clinic_id', 'left')->where('pr.status','Pending')->get();
        $lab_request_list = $get_lab_request->result();
        return $lab_request_list;
    }
    public function get_rad_request_list_filtered()
    {
        if ($this->input->post('status')) {
           ...
           $status = $this->input->post('status');
            //$status = "Pending";
            if ($status != 'all') {
                $cond = 'pr.status IN (SELECT status FROM patient_radiology_tests WHERE status="'. $status . '")';
            } else {
                $cond = '1=1';
            }
        }
        ...
        if ($this->input->post('date_range_from') != $today_date) {
            ...
            $date_range = array('DATE(pr.date_added) >=' => $first_date, 'DATE(pr.date_added) <=' => $second_date);
        } else {

           $date_range = array('DATE(pr.date_added)' => $today_date);

        }
        

        $get_lab_request = $this->db->select('pr.*,p.patient_title,p.patient_name,p.patient_status,p.patient_gender,p.patient_id_num,s.staff_firstname,s.staff_lastname,c.clinic_name, pr.id as request_id')->from('patient_radiology_tests pr')->join('patient_details as p', 'p.id=pr.patient_id', 'left')->join('staff as s', 's.user_id=pr.sender_id', 'left')->join('patient_vitals as pv', 'pv.patient_id=pr.patient_id', 'left')->join('clinics as c', 'c.id=pv.clinic_id', 'left')->where($cond)->where($date_range)->get();
        $lab_request_list = $get_lab_request->result();
        return $lab_request_list;
    }
    public function get_radiology_request_by_id2($id)
    {
    ...
    public function get_radiology_request_by_id3($id)
    {
        $get_lab_request = $this->db->select('pr.*,p.patient_title,p.patient_name,p.patient_status,p.patient_dob,p.patient_gender,p.patient_id_num,s.staff_firstname,s.staff_lastname,c.clinic_name, i.Name as rad_test_name')->from('patient_radiology_tests pr')->join('service_charge_items as i', 'i.id=pr.radiology_test_id', 'left')->join('patient_details as p', 'p.id=pr.patient_id', 'left')->join('staff as s', 's.user_id=pr.sender_id', 'left')->join('patient_vitals as pv', 'pv.patient_id=pr.patient_id', 'left')->join('clinics as c', 'c.id=pv.clinic_id', 'left')->where('pr.id', $id)->get();
        $lab_request_list = $get_lab_request->row();
        return $lab_request_list;
    }
    public function get_radiology_request_by_id($id)
    {
        $get_lab_request = $this->db->select('pr.*,p.patient_title,p.patient_name,p.patient_status,p.patient_dob,p.patient_gender,p.patient_id_num,s.staff_firstname,s.staff_lastname,c.clinic_name,prt.special_instuction')->from('patient_radiology_tests pr')->join('patient_details as p', 'p.id=pr.patient_id', 'left')->join('staff as s', 's.user_id=r.sender_id', 'left')->join('patient_vitals as pv', 'pv.patient_id=pr.patient_id', 'left')->join('clinics as c', 'c.id=pv.clinic_id', 'left')->group_by('r.radiology_test_unique_id')->where('pr.id', $id)->get();
        $lab_request_list = $get_lab_request->row();
        return $lab_request_list;
    }
    ...
    public function update_rad_request()
    {
        ...
        $data = array (
            ...
            $this->db->where('id', $radiology_test_id);
