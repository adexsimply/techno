...
class Nursing_m extends CI_Model
{
    ...
    public function get_appointment_vitals2()
    {
        ...
        $get_appointments = $this->db->select('pa.*,p.*,sa.title,c.clinic_name, pa.id as app_id')->from('patient_appointments pa')->join('patient_details as p', 'p.id=pa.patient_id', 'left')->join('salutations as sa', 'sa.id=p.patient_title', 'left')->join('clinics as c', 'c.id=pa.clinic_id', 'left')->order_by('pa.appointment_date', 'DESC')->order_by('pa.appointment_time', 'DESC')->where('pa.paid','Yes')->where('DATE(pa.appointment_date)',$curr_date)->where('pa.id NOT IN (SELECT appointment_id FROM patient_vitals WHERE appointment_id=pa.id)')->get();
        $appointment_list = $get_appointments->result();
        return $appointment_list;
    ...
    public function get_appointment_vitals()
    {
        ...
        $get_appointments = $this->db->select('pa.*,p.*,sa.title,c.clinic_name,s.staff_title,s.staff_firstname,s.staff_middlename,s.staff_lastname,pv.*, pa.id as app_id,pv.id as vital_id')->from('patient_appointments pa')->join('patient_details as p', 'p.id=pa.patient_id', 'left')->join('salutations as sa', 'sa.id=p.patient_title', 'left')->join('clinics as c', 'c.id=pa.clinic_id', 'left')->join('staff as s', 's.user_id=pa.doctor_id', 'left')->join('patient_vitals as pv', 'pv.appointment_id=pa.id', 'left')->where('pa.paid','Yes')->where($cond)->where($clinic_cond)->where($date_range)->order_by('pa.appointment_date', 'DESC')->order_by('pa.appointment_time', 'DESC')->get();
